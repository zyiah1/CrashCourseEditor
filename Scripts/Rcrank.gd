...
"                  unit_name: Point"]
	data += dataseg
	$crank.position = ($start.position + $end.position)/2
	$start.position = $end.position
	$start.frame = 1
	segments += 1
...
func _draw():
	if points.size() >= 2:
		$crank.position = (points[0].position + points[1].position)/2
	else:
		if $crank.position != $end.position:
			$crank.position = ($start.position + $end.position)/2
		if points.size() == 1:
			$crank.position = ($start.position + points[0].position)/2
